...
void launch_daemon() {
    ...
    openlog("mycron", LOG_PID, LOG_DAEMON);
}
...
state get_next_state(state current_state) {
    switch(current_state) {
        ...
        case state::args:   exit(EXIT_FAILURE);
...
pid_t run_job(Job& job) {
    ...
    if (pid == 0) {
        ...
        for (int i = 0; i < num_args; ++i) {
            argv[i] = job.args[i].data();
        }
        argv[num_args] = NULL;

        printf("[*] Successfully run the job!\n");

        execvp(job.command.data(), argv);
    }
...
int main()
{
    ...
    while (i < N_sec) {
        ...
        // Get the local time written into lt
    ...
    printf("[*] mycron daemon terminated\n");
